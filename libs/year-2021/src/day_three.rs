...
impl ColumnCount {
    ...
    fn get_binary(column_counts: HashMap<usize, ColumnCount>) -> String {
        let counts: Vec<&ColumnCount> = (0..WIDTH).into_iter()
...
fn calculate_epsilon_rate(bin: &String) -> isize {
    let inverse: String = bin.chars().map(|ch| {
        if ch == '1' {
            '0'
        } else {
            '1'
...
fn calculate_power_consumption() -> i32 {
    ...
    read_lines("resources/day_3.txt")
        .iter()
        .for_each(|line | {
            for (idx, ch) in line.chars().enumerate() {
                let entry = columns
                    .entry(idx)
                    .or_insert(ColumnCount::new());
